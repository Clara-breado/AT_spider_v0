...
#     get_url_thread = threading.Thread(target=get_url,args=(res_queue,))
#     get_url_thread.daemon = True
#     get_url_thread.start()
#     time.sleep(15)
...
#     thread_pool = []
#     for i in range(3):
#         use_url_thread = threading.Thread(target=use_url,args=(res_queue,out_queue,))
#         use_url_thread.daemon = True
#         use_url_thread.start()
#         thread_pool.append(use_url_thread)

#     for us_thread in thread_pool:
#         us_thread.join()

    # use_url_thread_1 = threading.Thread(target=use_url,args=(res_queue,out_queue,))
    # use_url_thread_1.daemon = True
    # use_url_thread_1.start()  

    # use_url_thread_2 = threading.Thread(target=use_url,args=(res_queue,out_queue,))
    # use_url_thread_2.daemon = True
    # use_url_thread_2.start()  

    # use_url_thread_3 = threading.Thread(target=use_url,args=(res_queue,out_queue,))
    # use_url_thread_3.daemon = True
    # use_url_thread_3.start()  

    # storage_url_thread = threading.Thread(target=storage_url,args=(out_queue,))
    # storage_url_thread.daemon = True
    # storage_url_thread.start()

    # time.sleep(15)  #!!!!time to wait the procs
    # print(out_queue.qsize())
    # while out_queue.empty() is False:
    #     print(out_queue.get()+"\n")
